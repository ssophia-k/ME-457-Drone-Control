...
from Models.sensors import MavDynamics as SensorsMavDynamics
...
from Estimators.khushant_observer import Observer
...
mav_sensors = SensorsMavDynamics(SIM.ts_simulation)
observer = Observer(SIM.ts_simulation, initial_state=mav.true_state, initial_measurements=sensors)
...
while sim_time < end_time:
    ...
    # sensor measurements
    mav_sensors.external_set_state(mav._state)
    mav_sensors._Va = mav._Va  
    mav_sensors._alpha = mav._alpha
    mav_sensors._beta = mav._beta
    mav_sensors._wind = mav._wind

    sensors = mav_sensors.sensors()

   # Update the observer with the sensor measurements
    estimated_state = observer.update(sensors)

    # autopilot (using estimated state instead of true state)
